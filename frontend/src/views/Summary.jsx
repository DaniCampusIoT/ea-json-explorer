...
// Mini conversor MD->HTML para el PDF (sin deps extra)
...
export default function Summary() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/blocks/${blockId}`)
      ...
      .catch(() => {
        ...
        if (proj) setBlock(proj.blocks.find(b => b.id === blockId) || null)
  ...
  function handleExportMD() {
    ...
    const parentName = window.eaProject?.idMap[block?.parentId || block?.parent_id]?.name
    ...
    <div style={{ maxWidth: '820px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: '0.75rem', marginBottom: '1.5rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.5rem', flexShrink: 0 }}>
          {canExport && (
            <>
              <button className="btn btn-ghost" style={{ fontSize: '0.78rem' }} onClick={handleExportMD}
                title="Exportar como Markdown">
                💾 .md
              </button>
              <button className="btn btn-ghost" style={{ fontSize: '0.78rem' }} onClick={handleExportPDF}
      ...
      {loadingImage && (
        <div className="card" style={{ marginBottom: '1.25rem', textAlign: 'center', padding: '2rem' }}>
          ...
          <p style={{ color: 'var(--color-text-muted)', fontSize: '0.875rem' }}>Generando imagen con GPT-IMAGE… (10-20 segundos)</p>
        </div>
      )}
